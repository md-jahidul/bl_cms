...
class PermissionTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $roles = Role::where('user_type','assetlite')->where('alias','!=','assetlite_super_admin')->get();

        $callback1 = function($role, $controller){
            return $role->alias == 'assetlite_super_user' || ( $controller != 'UserController' && $controller != 'RolesController' && $controller != 'PermissionsController');
        };

        $callback2 = function($role,$act){
            return ( $role->alias != 'assetlite_normal_user' ) || ( $act != 'edit' && $act != 'update' && $act != 'destroy' && $act != 'metaTagsEdit' );
        };

//      ====== Create Permissions ========
        foreach ($roles as $role){
            $this->createPermissions($actions["App\Http\Controllers\AssetLite"], $role, $callback1, $callback2 );
        }
