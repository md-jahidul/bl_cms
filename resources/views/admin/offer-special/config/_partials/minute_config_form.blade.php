                            ...
                            <div class="row">
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="minute_lower_price">Lower<small
                                                    class="text-danger">*</small></label>
                                        <input required type="number"
                                               id="minute_lower_price" min="1"
                                ...
                                <div class="col-md-4">
                                    <div class="form-group">
                                        <label for="minute_upper_price">Upper</label>
                                        <input required type="number"
                                               id="minute_upper_price"
                                               class="form-control minute_filter_input limit-input"
                                ...
                                <div class="col-md-1 add-button">
                                    <button type="button" id="add_minute_filter"
...
@push('page-js')
    <script>
        let minuteFilterTable = null;
        $(function () {

            let saveMinuteFilter = function (param) {
                return $.ajax({
                    url: '{{route('special-pack.filter.minute.save')}}',
                    method: 'post',
                    data: param
                });
            }
            // datatable for minute filter

            minuteFilterTable = $("#minute_filter_table").dataTable({
                scrollX: true,
                processing: true,
                searching: false,
                paging: false,
                ...
                ordering: false,
                ajax: {
                    url: '{{ route('special-pack.filter.minute.list') }}',
                },
                columns: [
                    {
                        name: 'sl',
...

                    {
                        ...
                        render: function (data, type, row) {
                            ...
                            <button type="button" class="btn btn-sm btn-icon btn-outline-danger minute-filter-del" data-id="` + row.id + `"><i class="la la-remove"></i></button>
                          </div>`
                        }
                    }
                ],
            ...
            $('#add_minute_filter').on('click', function (e) {
                var callSaveMinuteFilter;
                e.preventDefault();
                let lower_price = $("#minute_lower_price").val();
                let upper_price = $("#minute_upper_price").val();
                ...
                callSaveMinuteFilter = saveMinuteFilter(new function () {
                ...
                callSaveMinuteFilter.done(function (data) {
                    $('#minute_filter_table').DataTable().ajax.reload();
                    Swal.fire(
                        'Success!',
                        'Successfully Added',
                        'success',
                    );
                    $("#minute_lower_price").val('');
                    $("#minute_upper_price").val('');
            ...
            $(document).on('click', '.minute-filter-del', function (e) {
                e.preventDefault();
                let id = $(this).data('id');
                let call = null;
            ...
            $(document).on('input','.minute_filter_input',function () {
                let input = $(this).val();
