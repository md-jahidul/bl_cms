...
@section('title', 'LMS Components List')
@section('card_name', 'LMS Components List')
@section('breadcrumb')
    <li class="breadcrumb-item active"><strong>LMS Components List</strong></li>
@endsection
@section('action')
    <a href="" class="btn btn-primary round btn-glow px-2 create_component" data-toggle="modal" data-target="#large" role="button">
        <i class="la la-plus"></i>Add LMS Component
    </a>
@endsection
@section('content')
    <section>
                                ...
                                <td class="action">
                                    @if(isset($component['is_api_call_enable']))
                                        @if($component['is_api_call_enable'] == 0)
                                            <a href="{{ route("lms-components.status.update", $component['id']) }}" data-value="enable  {{ $component['title_en'] }}"
                                               class="btn btn-danger border-0 change_status" title="Click to enable">Disabled</a>
                                        @else
                                            <a href="{{ route("lms-components.status.update", $component['id']) }}" data-value="disable {{ $component['title_en'] }}"
                                        ...
                                        <a href="#" remove="{{ route("lms-components.destroy", $component['id']) }}" class="border-0 btn btn-danger delete_btn" data-id="{{ $component['id'] }}" title="Delete the component">
            ...
            <div class="modal-dialog modal-lg" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h4 class="modal-title" id="myModalLabel17">LMS Component</h4>
...
@push('page-js')
    <script>
        (function () {
            ...
            let versionCode = $("input[name='version_code']");
            let active = $("#active");
            ...
            function saveNewPositions()
            {
                ...
                $.ajax({
                    ...
                    error: function () {
                        alert('Opps, something went wrong!!')
                        window.location.replace("{{ url('lms-components') }}");
                    }
                });
            }
