@extends('layouts.admin')
@section('title', 'Application Details')
@section('card_name', 'Application Details')
@section('breadcrumb')
    <li class="breadcrumb-item "><a href="{{ route('application.list') }}"> Application List</a></li>
    <li class="breadcrumb-item active">Application Details</li>
@endsection
<style>
    body {
        font-family: 'Inter', sans-serif;
    }

    span {
        font-family: 'Inter', sans-serif;
    }

    .heading {
        color: #212121;
        text-align: right;
        font-size: 24px;
        font-style: normal;
        font-weight: 700;
        line-height: 120%;
    }

    .box-1 {
        width: 100%;
        padding: 18px;
        margin-top: 40px;
        border-radius: 8px;
        border: 1px solid #FBD1BD;
        border-collapse: separate;
        background: linear-gradient(90deg, #FFF 0%, #FFFAF8 100%);

    }

    .box-2 {
        width: 100%;
        border-radius: 8px;
        border: 1px solid #E0E0E0;
        padding: 18px;
    }

    .box-3 {
        width: 100%;
        border-radius: 8px;
        border: 1px solid #E0E0E0;
        padding: 12px;
    }

    .box-3 .item {
        border-bottom: 1px solid #E0E0E0;
        display: block;
        min-height: 40px;
    }

    .box-3 .item {
        margin-bottom: 10px;
    }

    .box-3 .item:last-child {
        border-bottom: none;
        margin-bottom: 0px;
    }

    .box-3 .left {
        float: left;
    }

    .box-3 .right {
        float: left;
        padding-left: 10px;
        margin-top: -4px;
    }

    .box-txt-1 {
        display: block;
        font-size: 15px;
        font-style: normal;
        font-weight: 400;
        color: #757575;
        margin-bottom: 5px;
        letter-spacing: 0.2px;
        font-family: 'Inter', sans-serif;
    }

    .box-txt-2 {
        font-size: 15px;
        font-style: normal;
        font-weight: 600;
        line-height: 130%;
        font-family: 'Inter', sans-serif;
    }

    .txt-1 {
        display: block;
        font-size: 15px;
        font-style: normal;
        font-weight: 400;
        color: #757575;
        margin-bottom: 5px;
        letter-spacing: 0.2px;
        font-family: 'Inter', sans-serif;
        padding-left: 20px;
        padding-top: 20px;
    }

    .txt-2 {
        font-size: 15px;
        font-style: normal;
        font-weight: 600;
        line-height: 130%;
        font-family: 'Inter', sans-serif;
        padding-left: 20px;
    }

    .attach-1 {
        display: block;
        font-size: 15px;
        font-style: normal;
        font-weight: 600;
        color: #757575;
        margin-bottom: 5px;
        letter-spacing: 0.2px;
        font-family: 'Inter', sans-serif;
    }

    .attach-2 {
        font-size: 15px;
        font-style: normal;
        font-weight: 600;
        line-height: 130%;
        font-family: 'Inter', sans-serif;
    }

    .txt-3 {
        color: #212121;
        font-family: 'Inter', sans-serif;
        font-size: 14px;
        font-style: normal;
        font-weight: 700;
        line-height: 130%;
        letter-spacing: 0.28px;
    }

    .txt-4 {
        color: #616161;
        display: block;
        font-family: 'Inter', sans-serif;


        font-size: 15px;
        font-style: normal;
        font-weight: 400;
        line-height: 125%;
        letter-spacing: 0.2px;
    }

    .txt-5 {

        color: #757575;
        margin-top: 8px;
        font-family: 'Inter', sans-serif;

        font-size: 15px;
        font-style: normal;
        font-weight: 400;
        line-height: 125%;
        letter-spacing: 0.2px;

    }

    .txt-6 {
        color: #757575;

        font-size: 15px;
        font-style: normal;
        font-weight: 400;
        line-height: 125%;
        display: block;
        margin-bottom: 6px;
        letter-spacing: 0.2px;
    }

    .txt-7 {
        color: #212121;
        font-size: 15px;
        font-style: normal;
        font-weight: 700;
        line-height: 125%;
        letter-spacing: 0.24px;
    }

    .startup-info .item{
        margin-bottom: 12px;
    }
    .personal-info tr:last-child .txt-2{
        margin-bottom: 0px;
    }
    .personal-info .txt-2{
        margin-bottom: 16px;
    }
</style>
@section('action')
    <a href="{{ route('application.list') }}" class="btn btn-warning  btn-glow px-2"><i class="la la-list"></i> Back </a>
@endsection
@section('content')
    <div class="row justify-content-md-center">
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    <table style="width: 100%;">
                        <tr>
                            <td><img src="data:image/png;base64,"/></td>
                            <td style="float:right;">
                                <h1 class="heading">Submitted Idea</h1>
                                <div style="float:right; width: 410px;">
                                    @if(!empty($data['submitted_date']))
                                        <span style="float: left;border-right: 1px solid #E0E0E0; text-align: right; padding-right: 20px;">
                                            <span style="
                                              display: block;
                                              font-size: 17px;
                                              font-style: normal;
                                              font-weight: 400;
                                              color: #757575;
                                              line-height: 125%; /* 12.5px */
                                              letter-spacing: 0.2px;">Submitted on</span>
                                            <span style="
                                              font-size: 17px;
                                              font-weight: 600;
                                              line-height: 130%;">{{ $data['submitted_date'] ?? null }}</span>
                                        </span>
                                    @else
                                        <span style="float: left;border-right: 1px solid #E0E0E0; text-align: right; padding-right: 20px;">
                                            <span style="
                                              display: block;
                                              font-size: 17px;
                                              font-style: normal;
                                              font-weight: 400;
                                              color: #757575;
                                              line-height: 125%; /* 12.5px */
                                              letter-spacing: 0.2px;">Application Status: </span>
                                            <span style="
                                              font-size: 17px;
                                              font-weight: 600;
                                              line-height: 130%;"><strong>Draft</strong></span>
                                        </span>
                                    @endif

                                    <span style="float:right; padding-left: 16px; text-align: right;">
                                        <span style="
                                          display: block;
                                          font-size: 17px;
                                          font-style: normal;
                                          font-weight: 700;
                                          color: #757575;
                                          line-height: 125%; /* 12.5px */
                                          letter-spacing: 0.2px;">ID Number</span>
                                        <span style="
                                          font-size: 17px;
                                          color:#F16522;
                                          font-style: normal;
                                          font-weight: 700;
                                          line-height: 130%;">IDN. #{{ $data['application_id'] ?? null }}</span>
                                    </span>
                                </div>
                            </td>
                        </tr>
                    </table>

                    <table class="box-1">
                        <tr>
                            <td>
                                <div class="box-txt-1">Idea Name</div>
                                <div class="box-txt-2">{{ $data['summary']['idea_title'] ?? null }}</div>
                            </td>
                            <td>
                                <div class="box-txt-1">Industry</div>
                                <div class="box-txt-2">{{ $data['summary']['industry'] ?? null }}</div>
                            </td>
                        </tr>
                    </table>
                    <h4 class="mt-2"><strong>Personal Information</strong></h4>
                    <table class="box-2 personal-info">
                        <tr>
                            <td>
                                <div class="txt-1">Name</div>
                                <div class="txt-2">{{ $data['personal']['name'] ?? null }}</div>
                            </td>
                            <td>
                                <div class="txt-1">Gender</div>
                                <div class="txt-2">{{ $data['personal']['gender'] ?? null }}</div>
                            </td>
                            <td>
                                <div class="txt-1">Designation</div>
                                <div class="txt-2">{{ $data['personal']['designation'] ?? null }}</div>
                            </td>
                        </tr>

                        <tr>
                            <td>
                                <div class="txt-1">Email</div>
                                <div class="txt-2">{{ $data['personal']['email'] ?? null }}</div>
                            </td>
                            <td>
                                <div class="txt-1">Contact Number</div>
                                <div class="txt-2">{{ $data['personal']['phone_number'] ?? null }}</div>
                            </td>
                        </tr>


                        <tr>
                            <td>
                                <div class="txt-1">Profession</div>
                                <div class="txt-2">{{ $data['personal']['profession'] ?? null }}</div>
                            </td>
                            <td>
                                <div class="txt-1">Institute/Organization</div>
                                <div class="txt-2">{{ $data['personal']['institute_or_org'] ?? null }}</div>
                            </td>
                            <td>
                                <div class="txt-1">Education</div>
                                <div class="txt-2">{{ $data['personal']['education'] ?? null }}</div>
                            </td>
                        </tr>
                    </table>
                    <h4 class="mt-2"><strong>Team Members</strong></h4>
                    <table class="table tab-content table-bordered box-2">
                        <tr>
                            @if(isset($data['personal']['team_members']))
                                @foreach($data['personal']['team_members'] as $member)
                                    <td>
                                        <div class="txt-3">{{ $member['name'] ?? null }}</div>
                                        <div class="txt-4">{{ $member['designation'] ?? null }}
                                        </div><div class="txt-5">{{ $member['email'] ?? null }}</div>
                                    </td>
                                @endforeach
                            @endif
                        </tr>
                    </table>
                    <h4 class="mt-2"><strong>Startup Information</strong></h4>
                    <div class="box-2 startup-info">
                        <div class="item">
                            <div class="txt-6">
                                Problem Identification
                            </div>
                            <div class="txt-7">
                                {{ $data['startup']['problem_identification'] ?? null }}
                            </div>
                        </div>
                        <div class="item">
                            <div class="txt-6">
                                The Big Idea
                            </div>
                            <div class="txt-7">
                                {{ $data['startup']['big_idea'] ?? null }}
                            </div>
                        </div>
                        <div class="item">
                            <div class="txt-6">
                                Target Group
                            </div>
                            <div class="txt-7">
                                {{ $data['startup']['target_group'] ?? null }}
                            </div>
                        </div>
                        <div class="item">
                            <div class="txt-6">
                                Market Size
                            </div>
                            <div class="txt-7">
                                {{ $data['startup']['market_size'] ?? null }}
                            </div>
                        </div>
                        <div class="item">
                            <div class="txt-6">
                                Business Model
                            </div>
                            <div class="txt-7">
                                {{ $data['startup']['business_model'] ?? null }}
                            </div>
                        </div>
                        <div class="item">
                            <div class="txt-6">
                                GTM Plan
                            </div>
                            <div class="txt-7">
                                {{ $data['startup']['gtm_plan'] ?? null }}
                            </div>
                        </div>
                        <div class="item">
                            <div class="txt-6">
                                Financial Metrics
                            </div>
                            <div class="txt-7">
                                {{ $data['startup']['financial_metrics'] ?? null }}
                            </div>
                        </div>


                        <div class="item">
                            <div class="txt-6">
                                Is there any existing product or service for this problem?*
                            </div>
                            <div class="txt-7">
                                {{ isset($data['startup']['exist_product_service']) && $data['startup']['exist_product_service'] ? "Yes" : "No" }}
                            </div>
                        </div>

                        {{-- If Yes  Start--}}
                        @if(isset($data['startup']['exist_product_service']))
                            <div class="item">
                                <div class="txt-6">
                                    Short Description
                                </div>
                                <div class="txt-7">
                                    {{ $data['startup']['exist_product_service_details'] ?? null }}
                                </div>
                            </div>
                            <div class="item">
                                <div class="txt-6">
                                    How is your product/service different from the existing one?*
                                </div>
                                <div class="txt-7">
                                    {{ $data['startup']['exist_product_service_diff'] ?? null }}
                                </div>
                            </div>
                        @endif
                        {{-- If Yes  End--}}

                        <div class="item">
                            <div class="txt-6">
                                Have you received any funding?*
                            </div>
                            <div class="txt-7">
                                {{ isset($data['startup']['receive_fund']) && $data['startup']['receive_fund'] ? "Yes" : "No" }}
                            </div>
                        </div>
                        {{-- If Yes received funding Start--}}
                        @if(isset($data['startup']['receive_fund']))
                            <div class="item">
                                <div class="txt-6">
                                    What is the source of your funding?
                                </div>
                                <div class="txt-7">
                                    {{ $data['startup']['receive_fund_source'] ?? null }}
                                </div>
                            </div>
                        @endif
                        {{-- If Yes received funding End--}}

                        <div class="item">
                            <div class="txt-6">
                                Which stage is your startup currently in?
                            </div>
                            <div class="txt-7">
                                {{ $data['startup']['startup_current_stage'] ?? null }}
                            </div>
                        </div>
                    </div>
                    <h4 class="mt-2"><strong>Attachments</strong></h4>
                    <div class="box-3">
                        @foreach($data['attachments'] as $attachment)
                            <div class="item">
                                <a href="{{ config('filesystems.file_base_url') . $attachment['file_path'] }}">
                                    <div class="left">
                                        <img src="data:image/png;base64," />
                                    </div>
                                    <div class="right">
                                        <span class="attach-2">{{ $attachment['file_name'] ?? null }}</span>
                                        <span class="attach-1">File Size:<span style="font-weight: 600; color: #212121; margin-left:5px">{{ $attachment['file_size'] ?? null }}</span></span>
                                    </div>
                                </a>
                            </div>
                        @endforeach
                    </div>
                    <table style="width: 100%; margin-top: 24px;">
                        <tr>
                            <td><span style="float: left; margin-right: 8px;"><img src="data:image/png;base64," /></span>
                                <span style="float: left;">
                                    <h5 style="margin:0; color: #212121;

                                         font-size: 12px;
                                         font-style: normal;
                                         font-weight: 600;
                                         line-height: 130%;">I consent to BL Labs sharing my information</h5>
                                    <p style="color: #616161;

                                         font-size: 15px;
                                         font-style: normal;
                                         font-weight: 400;
                                         line-height: 125%;
                                         letter-spacing: 0.2px;">
                                        I consent to BL Labs sharing the demographic information I provided above
                                        and my contact
                                        information to third-party partners who are seeking to help
                                    </p>
                                </span>
                            </td>
                        </tr>
                    </table>
                </div>
            </div>
        </div>
    </div>
@stop

@push('page-js')

@endpush
