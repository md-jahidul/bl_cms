...
@section('title', 'Navigation Rail List')
@section('card_name', 'Navigation Rail')
@section('breadcrumb')
    <li class="breadcrumb-item active">Navigation Rail List</li>
@endsection
                    ...
                    <table class="table table-striped table-bordered" role="grid" aria-describedby="Example1_info" style="">
                        ...
                        @foreach ($navigationMenus as $data)
                            <tr data-index="{{ $data->id }}" data-position="{{ $data->display_order }}">
                                ...
                                <td class="action">
                                    <a href="{{ route('heme-navigation-rail.edit', $data->id) }}" role="button"
                                       class="btn btn-outline-info border-0"><i class="la la-pencil" aria-hidden="true"></i></a>
                                    <a href="#" remove="{{ route('heme-navigation-rail.destroy', $data->id) }}"
...
@push('page-js')
    ...
    <script>
        var auto_save_url = "{{ url(route('navigation-rail.sort')) }}";
