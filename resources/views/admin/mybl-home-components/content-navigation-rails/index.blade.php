...
@section('title', 'Content Navigation Rail List')
@section('card_name', 'Content Navigation Rail')
@section('breadcrumb')
    <li class="breadcrumb-item active">Content Navigation Rail List</li>
@endsection

@section('content_header')
    <h1 class="content-header-title mb-0 d-inline-block">
        Content Navigation Rail List
    </h1>
@endsection
...
@section('content')
    ...
    <section>
        <form
            action=" @if(isset($navigationMenu)) {{route('content-navigation-rail.update',$navigationMenu->id)}} @else {{route('content-navigation-rail.store')}} @endif "
                    ...
                    <table class="table table-striped table-bordered" role="grid" aria-describedby="Example1_info" style="">
                        ...
                        @foreach ($navigationMenus as $data)
                            <tr data-index="{{ $data->id }}" data-position="{{ $data->display_order }}">
                                ...
                                <td class="action">
                                    <a href="{{ route('content-navigation-rail.edit', $data->id) }}" role="button"
                                       class="btn btn-outline-info border-0"><i class="la la-pencil" aria-hidden="true"></i></a>
                                    <a href="#" remove="{{ route('content-navigation-rail.destroy', $data->id) }}"
...
@push('page-js')
    ...
    <script>
        var auto_save_url = "{{ url(route('content-navigation-rail.sort')) }}";
