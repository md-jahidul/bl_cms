    ...
    @php
        ...
        for($i = count($menu_items) - 1; $i >= 0; $i--){
            $liHtml .=  $i == 0 ? '<li class="breadcrumb-item active">' .  $menu_items[$i]['en_label_text']  . '</li>' :
                                  '<li class="breadcrumb-item"><a href="'. url("mybl-menu/". $menu_items[$i]["id"] . "/child-menu") .'">' .  $menu_items[$i]['en_label_text']  . '</a></li>';
        }
    @endphp
    {!! $liHtml !!}
@endsection
        ...
        <div class="card col-sm-12">
            <div class="card-content collapse show">
                <div class="card-body card-dashboard">
                    <h4 class="menu-title">{{ !empty($menu_items) ? "Sub Categories" : 'Categories' }} List</h4>
                        ...
                        @if(count($menus) == !0)
                            @foreach($menus as $menu)
                                ...
                                <tr data-index="{{ $menu->id }}" data-position="{{ $menu->display_order }}">
                                   ...
                                   <td class="text-center" width="10%"><a href="{{ url("mybl-menu/$menu->id/child-menu") }}" class="btn btn-outline-success">Child Menus <span class="ml-1 badge badge-pill badge-default badge-danger badge-default badge-up badge-glow">{{ $childNumber }}</span></a></td>
                                </tr>
                            @endforeach
                        @else
