                                ...
                                <td class="action">
                                    @if(isset($component['is_api_call_enable']))
                                        @if($component['is_api_call_enable'] == 0)
                                            <a href="{{ route("nonbl.components.status.update", $component['id']) }}" data-value="enable  {{ $component['title_en'] }}"
                                               class="btn btn-danger border-0 change_status" title="Click to enable">Disabled</a>
                                        @else
                                            <a href="{{ route("nonbl.components.status.update", $component['id']) }}" data-value="disable {{ $component['title_en'] }}"
    ...
    <script>
        (function () {
            ...
            $('.create_component').click(function () {
                $('#form').prop('action', "{{ route('nonbl.components.store') }}")
                $('#submit').text('Save')
                titleEn.val('');
                titleBn.val('');
            ...
            $('.edit').click(function () {
                ...
                $.ajax({
                    url: "{{ url('non-bl-components/edit') }}/" + componentID,
                    methods: "get",
            ...
            function saveNewPositions()
            {
                ...
                $.ajax({
                    methods: "POST",
                    url: "{{ url('non-bl-components-sort') }}",
                    data: {
                        position: positions
                    },
                    success: function (data) {
                    ...
                    error: function () {
                        alert('Opps, something went wrong!!')
                        window.location.replace("{{ url('non-bl-components') }}");
                    }
                });
            }
