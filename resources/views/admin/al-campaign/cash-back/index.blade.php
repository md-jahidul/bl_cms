...
@section('action')
    <a href="{{route('al-cash-back-campaign.create')}}" class="btn btn-primary round btn-glow px-2"><i
            class="la la-plus"></i>
        Create Campaign
    </a>
@endsection
                    ...
                    <table class="table table-striped table-bordered alt-pagination no-footer dataTable" id="Example1"
                        ...
                        @foreach ($cashBackCampaigns as $data)
                            <tr>
                                ...
                                <td>{{$data->end_date}}</td>
                                <td>
                                    <a href="{{ route('al-cash-back-campaign.edit', [$data->id]) }}" role="button" class="btn-sm btn-outline-info border-0"><i class="la la-pencil" aria-hidden="true"></i></a>
                                    <a href="#" remove="{{ url("al-cash-back-campaign/destroy/$data->id") }}" class="border-0 btn-sm btn-outline-danger delete_btn" data-id="{{ $data->id }}" title="Delete">
