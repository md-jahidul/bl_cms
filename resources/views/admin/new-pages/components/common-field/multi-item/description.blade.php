...
<div class="form-group col-md-6">
    ...
    <textarea type="text" rows="3" name="{{ $fieldNameEn }}" class="form-control">{{ $data['desc']['value_en'] ?? '' }}</textarea>
...
<div class="form-group col-md-6">
    ...
    <textarea type="text" rows="3" name="{{ $fieldNameBn }}" class="form-control">{{ $data['desc']['value_bn'] ?? '' }}</textarea>
{{--    <input type="hidden" name="componentData[{{$key}}][desc][group]" value="{{ $key + 1 }}">--}}
    <input type="hidden" name="{{ $fieldNameID }}" value="{{ $data['desc']['id'] ?? '' }}">
</div>
