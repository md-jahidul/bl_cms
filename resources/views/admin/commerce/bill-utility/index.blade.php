...
@section('action')
    <a href="{{route('commerce-bill-utility.create')}}" class="btn btn-primary round btn-glow px-2"><i class="la la-plus"></i>
                        ...
                        <tbody id="sortable">
                            @foreach ($billUtilities as $utility)
                                <tr data-index="{{ $utility->id }}" data-position="{{ $utility->display_order }}">
                                    ...
                                    <td>
                                        <a href="{{ route('commerce-bill-utility.edit', $utility->id) }}" role="button"
...
@push('page-js')
    ...
    <script>
        var auto_save_url = "{{ url('commerce-bill-utility/sort-auto-save') }}";
        ...
        $(function () {
            $('.delete').click(function () {
                ...
                Swal.fire({
                    ...
                    if (result.value) {
                        $.ajax({
                            url: "{{ url('commerce-bill-utility/destroy') }}/"+id,
                            methods: "get",
                            success: function (res) {
                                ...
                                function redirect() {
                                    window.location.href = "{{ url('commerce-bill-utility') }}"
                                }
                            }
                        })
                    }
