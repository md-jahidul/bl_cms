...
@section('title', 'Ad Tech')
@section('card_name', 'Ad Tech')
@section('action')
    <a href="{{route('ad-tech.create')}}" class="btn btn-info btn-glow px-2">
        Add Image
    </a>
@endsection
            ...
            <div class="card-content collapse show">
                <div class="card-body card-dashboard">
                    <table class="table table-striped table-bordered">
                        ...
                        <tbody id="sortable">
                        @foreach($adTechs as $index=>$slider_image)
                            <tr data-index="{{ $slider_image->id }}" data-position="{{ $slider_image->sequence }}">
                                <td width="3%"><i class="icon-cursor-move icons"></i></td>
                                <td>{{ $slider_image->id }}</td>
                                <td><img class="" src="{{ asset($slider_image->image_url) }}" alt="Ad Tech Image"
                                ...
                                <td class="action">
                                    <form action="{{route('ad-tech.destroy',$slider_image->id)}}"
                                          id="del_form_{{$slider_image->id}}"
                                          method="post">
                                        @csrf
                                        @method('delete')
                                        <a href="{{route('ad-tech.edit', $slider_image->id )}}"
...
@push('page-js')
    <script>
        let auto_save_url = "{{ url('ad-tech/addImage/update-position') }}";
