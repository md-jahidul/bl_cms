        ...
        <div class="card">
            <div class="card-content collapse show">
                <div class="card-body card-dashboard">
                    <div class="card-body card-dashboard">
                        <form role="form" action="{{ route("business-types.update", $businessTypes->id) }}" method="POST" novalidate>
                            @method('PUT')
                            @csrf
                            <input type="hidden" name="id" value="{{ $businessTypes->id }}">
                            <div class="row">
                                <div class="form-group col-md-6 {{ $errors->has('title_en') ? ' error' : '' }}">
                                    <label for="title_en" class="required">Title (English)</label>
                                    <input type="text" name="title_en"  class="form-control" placeholder="Enter title (english)"
                                           value="{{ $businessTypes->title_en }}" required data-validation-required-message="Enter slider title (english)">
                                ...
                                <div class="form-group col-md-6 {{ $errors->has('title_bn') ? ' error' : '' }}">
                                    ...
                                    <input type="text" name="title_bn"  class="form-control" placeholder="Enter title (english)"
                                           value="{{ $businessTypes->title_bn }}" required data-validation-required-message="Enter slider title (english)">
                                    <div class="help-block"></div>
...
{{--                                    <input type="text" name="url_slug_en" class="form-control" value="{{ isset($businessTypes->url_slug_en) ? $businessTypes->url_slug_en : null }}"--}}
...
{{--                                    <input type="text" name="url_slug_bn" class="form-control" value="{{ isset($businessTypes->url_slug_bn) ? $businessTypes->url_slug_bn : null }}"--}}
...
{{--                                    <textarea class="form-control" rows="7" name="page_header">{{ isset($businessTypes->page_header) ? $businessTypes->page_header : null }}</textarea>--}}
{{--                                    <small class="text-info">--}}
...
{{--                                    <textarea class="form-control" rows="7" name="page_header_bn">{{ isset($businessTypes->page_header_bn) ? $businessTypes->page_header_bn : null }}</textarea>--}}
{{--                                    <small class="text-info">--}}
{{--                                        <strong>Note: </strong> Title, meta, canonical and other tags--}}
{{--                                    </small>--}}
{{--                                </div>--}}
...
{{--                                    <textarea class="form-control" rows="7" name="schema_markup">{{ isset($businessTypes->schema_markup) ? $businessTypes->schema_markup : null }}</textarea>--}}
{{--                                    <small class="text-info">--}}
                                ...
                                <div class="form-group col-md-6 mb-2">
                                    ...
                                    <div class="form-group {{ $errors->has('status') ? ' error' : '' }}">
                                        <input required type="radio" name="status" value="1"
                                               ...
                                               id="input-radio-15" {{ ($businessTypes->status) ? 'checked' : '' }}>
                                        <label for="input-radio-15" class="mr-3">Active</label>
                                        <input required type="radio" name="status" value="0"
                                               data-validation-required-message="Please select status"
                                               id="input-radio-16" {{ ($businessTypes->status == 0) ? 'checked' : '' }}>
