...
@section('card_name', 'Generic Components Items List')
@section('breadcrumb')
...
<a href="{{ url('generic-components') }}" class="btn btn-warning round btn-glow px-2 create_component"><strong>Component List</strong></a>
@endsection
@section('content')
<section>
    <div class="card">
                ...
                <table class="table table-striped table-bordered">
                    ...
                    <tr width="3%" data-index="{{ $component['id'] }}" data-position="{{ $component['display_order'] }}"
                        ...
                        <td class="action">
                            ...
                            <a href="{{ route("generic-component-items.status.update", $component['id']) }}" data-value="enable  {{ $component['title_en'] }}"
                            ...
                            @endif
                            @if($component['component_key'] != null)
                    ...
                    <div class="modal-body">
                        <div class="card-body card-dashboard">
                            <div class="row">
                                ...
                                <input type="hidden" name="generic_component_id" value="{{ $id }}">
                            </div>
                        </div>
                    </div>
...
<script>
    (function () {
        ...
        // let genericComponentId = $("input[name='generic_component_id']");
        let active = $("#active");
        ...
        $('.create_component').click(function () {
            $('#form').prop('action', "{{ route('generic-component-items.store') }}")
            $('#submit').text('Save')
            titleEn.val('');
            titleBn.val('');
        ...
        $('.edit').click(function () {
            $('#form').prop('action', "{{ route('generic-component-items.update') }}")
            $('#submit').text("Update")
            ...
            $.ajax({
                url: "{{ url("generic-component-items/edit") }}/" + componentID,
                methods: "get",
                success: function (data) {
                    ...
                    // genericComponentId.val(data.generic_component_id)
        ...
        function saveNewPositions()
        {
            ...
            $.ajax({
                methods: "POST",
                url: "{{ url('generic-component-items-sort') }}",
                data: {
                    position: positions
                },
                success: function (data) {
                ...
                error: function () {
                    alert('Opps, something went wrong!!')
                    window.location.replace("{{ url('generic-component/' . $id . '/items') }}");
                }
            });
        }
