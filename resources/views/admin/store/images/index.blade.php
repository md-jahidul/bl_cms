...
@endsection
@section('action')
    <a href="{{route('appslider.images.create',$slider_information->id)}}" class="btn btn-info btn-glow px-2">
        Add Image
    </a>
                    ...
                    <table class="table table-striped table-bordered">
                        ...
                        @foreach($slider_information->sliderImages as $index=>$slider_image)
                            <tr data-index="{{ $slider_image->id }}" data-position="{{ $slider_image->sequence }}">
                                ...
                                <td class="action">
                                    <form action="{{route('appslider.images.destroy',$slider_image->id)}}"
                                          id="del_form_{{$slider_image->id}}"
                                          method="post">
                                        @csrf
                                        @method('delete')
                                        <a href="{{route('appslider.images.edit', $slider_image->id )}}"
...
@push('page-js')
    <script>
        ...
        let auto_save_url = "{{ url('appsliderImage/addImage/update-position') }}";
