...
class Kernel extends ConsoleKernel
{
    ...
    protected function schedule(Schedule $schedule)
    {
        ...
        $schedule->command('product-data:schedule')->withoutOverlapping()->everyMinute();
        ...
        $schedule->command('send:rafm-report-cs-sefcare')->withoutOverlapping()
            ->dailyAt(config('constants.cs_selfcare.cs_report_send_at'))
            ->timezone('Asia/Dhaka');
        $schedule->command('mybl:global-setting-scheduler')->withoutOverlapping()->everyThirtyMinutes();
    }
