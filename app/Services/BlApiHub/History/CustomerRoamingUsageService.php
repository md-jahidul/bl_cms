...
/**
 * Class CustomerRoamingUsageService
...
class CustomerRoamingUsageService extends BaseService
{
            ...
            foreach ($data->data as $usage_data) {
                ...
                if ($item) {
                    $call_data [] = [
                        ...
                        'is_outgoing' => $item->transactionType == 'Outgoing call',
    ...
    public function getDataUsageHistory($user, $from, $to)
    {
        if (!$user) {
            return $this->responseFormatter->sendErrorResponse("User not found", [
                'message' => "MSISDN NOT FOUND!"
    ...
    public function getSummaryUsageHistory($user, $from, $to)
    {
        if (!$user) {
            return $this->responseFormatter->sendErrorResponse("User not found", [
                'message' => "MSISDN NOT FOUND!"
        ...
        $redis_key = "roaming_usage_summary:" . $customer_id . ':' . $from . '-' . $to;
        $ttl_settings = MyBlAppSettings::where('key', MyBlAppSettingsKey::USAGE_HISTORY_TTL_SECONDS)->first();
        ...
        if (!$roaming_usage_summary = Redis::get($redis_key)) {
            $data = $this->prepareSummaryUsageData($customer_id, $from, $to);
